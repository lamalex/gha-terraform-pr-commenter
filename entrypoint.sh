...
if [[ $COMMAND == 'plan' ]]; then
  ...
  if [[ $EXIT_CODE -eq 0 || $EXIT_CODE -eq 2 ]]; then
    CLEAN_PLAN=$(echo "$INPUT" | sed -r '/Plan: /q') # Ignore everything after plan summary
    echo $CLEAN_PLAN
...
\`\`\`
  ...
  curl -v -sS -X POST -H "$AUTH_HEADER" -H "$ACCEPT_HEADER" -H "$CONTENT_HEADER" -d "$PR_PAYLOAD" -L "$PR_COMMENTS_URL"
